...
$LibcPayloadName = 'OESDK-LibC-0.0.13.zip'
...
function Select-LibcPayload {
    $candidates = @(
        (Join-Path ([Environment]::CurrentDirectory) $LibcPayloadName),
        (Join-Path (Get-Location).Path $LibcPayloadName),
        (Join-Path ([Environment]::GetFolderPath('UserProfile')) ("Downloads\\" + $LibcPayloadName))
    )
    foreach ($candidate in $candidates) {
        if (Test-Path -LiteralPath $candidate) { return [IO.Path]::GetFullPath($candidate) }
    }
    $dialog = New-Object Windows.Forms.OpenFileDialog
    $dialog.Title = 'Select the OESDK 0.0.13 libc payload'
    $dialog.Filter = 'OESDK libc package|OESDK-LibC-0.0.13.zip|ZIP packages|*.zip'
...
function Install-LibcPayload {
    ...
    $StatusLabel.Text = 'Verifying the OESDK 0.0.13 libc payload...'
    [Windows.Forms.Application]::DoEvents()
    ...
    if ($actual -ne $LibcPayloadSha256) {
        throw 'The OESDK 0.0.13 libc payload failed its SHA-256 security check.'
    }
...
function Install-OESDK {
    ...
    try {
        ...
        $releaseStatusProperty = $manifest.PSObject.Properties['releaseStatus']
        $releaseStatus = if ($null -ne $releaseStatusProperty) { [string]$releaseStatusProperty.Value } else { '' }
        if (-not [string]::IsNullOrWhiteSpace($releaseStatus) -and $releaseStatus -ne 'ready') {
            $releaseMessageProperty = $manifest.PSObject.Properties['releaseMessage']
            $releaseMessage = if ($null -ne $releaseMessageProperty) { [string]$releaseMessageProperty.Value } else { '' }
        ...
        Install-LibcPayload $stageRoot $StatusLabel
        ...
        $StatusLabel.Text = "OESDK 0.0.13 installed successfully."
    } finally {
...
[void](New-Label $form 'OESDK 0.0.13 Online Installer' 24 20 610 30)
...
[void](New-Label $form 'Author name:' 26 167 130)
$authorBox = New-TextBox $form '' 156 164 470
[void](New-Label $form 'Author email:' 26 202 130)
$emailBox = New-TextBox $form '' 156 199 470
[void](New-Label $form 'OS licence:' 26 237 130)
$licenseBox = New-TextBox $form 'MIT' 156 234 470

$progress.Location = New-Object Drawing.Point(26, 282)
$progress.Size = New-Object Drawing.Size(600, 22)
[void]$form.Controls.Add($progress)
$status = New-Label $form 'Ready to download OESDK.' 26 312 600

$installButton = New-Object Windows.Forms.Button
$installButton.Text = 'Install'
$installButton.Location = New-Object Drawing.Point(438, 377)
$installButton.Size = New-Object Drawing.Size(90, 30)
[void]$form.Controls.Add($installButton)
$cancelButton = New-Object Windows.Forms.Button
$cancelButton.Text = 'Cancel'
$cancelButton.Location = New-Object Drawing.Point(536, 377)
...
$installButton.Add_Click({
    ...
    try {
        ...
        $details = "OESDK Setup 0.0.13`r`n$([DateTime]::Now.ToString('O'))`r`n$($_ | Out-String)"
