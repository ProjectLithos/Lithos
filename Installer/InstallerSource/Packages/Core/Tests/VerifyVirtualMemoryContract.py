...
'OESDK_VIRTUAL_FLAG_NO_EXECUTE','OESDK_VIRTUAL_FLAGS_ALL','SplitTwoMibEntry','invlpg','mov %%cr3','EFER_NXE']
...
assert 'uint64_t Flags' in h
assert 'PageCount > (size_t)(UINTPTR_MAX / OESDK_VIRTUAL_PAGE_SIZE)' in c
assert 'Address > UINTPTR_MAX - (MappedBytes - 1U)' in c
assert '(Flags & ~OESDK_VIRTUAL_FLAGS_ALL) != 0U' in c
